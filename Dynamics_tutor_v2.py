...
if st.session_state.page == "landing":
    ...
    st.write("실시간 시뮬레이션을 통해 속도와 가속도의 유도 과정을 학습하세요.")
...
# --- Page 2: Problem Solving Chat (기존 유지) ---
elif st.session_state.page == "chat":
    # ... (기존과 동일하므로 생략하지만 실제 파일에는 포함되어야 함)
    pass

# --- Page 3: Interactive Lecture Agent (수정된 핵심 파트) ---
    ...
    with col_sim:
        st.subheader("🛠️ Live Parameters & Derivation")
        params = {}
        ...
        if topic == "Projectile Motion":
            ...
            st.info("💡 곡률 반경과 속도의 변화가 법선 가속도에 미치는 영향을 관찰하세요.")
        ...
        # render_v2_GitHub.py의 새로운 시각화 함수 호출
    ...
    with col_chat:
        st.subheader("💬 Socratic Tutor: Derivation Guide")
        if "lecture_session" not in st.session_state or st.session_state.lecture_session is None:
            # 수식 유도 과정을 중점적으로 설명하도록 프롬프트 강화
            sys_msg = (
                f"You are a Physics Professor at TAMUCC. Topic: {topic}. "
                "Explain the derivation of velocity and acceleration. "
                "1. Focus on how components are formed. 2. Use values from the sliders. "
                "3. For 'Normal & Tangent', prove why an = v^2/rho geometrically. "
                "4. Ask one guiding question at a time."
            )
            model = get_gemini_model(sys_msg)
            st.session_state.lecture_session = model.start_chat(history=[])
            st.session_state.lecture_session.send_message(
                f"안녕하세요 {st.session_state.user_name} 학생

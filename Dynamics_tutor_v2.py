...
# 2. CSS to Unify Button Heights for a Clean Grid
st.markdown("""
    <style>
    div.stButton > button {
        height: 100px;
...
# --- Page 1: Main Menu (Split Version) ---
if st.session_state.page == "landing":
    ...
    with col_l1:
        if st.button("🎓 Lecture:\nProjectile Motion", key="lec_proj", use_container_width=True):
            st.session_state.lecture_topic = "Projectile Motion"
            st.session_state.page = "lecture"; st.rerun()
            
    with col_l2:
        if st.button("🎓 Lecture:\nNormal & Tangent", key="lec_nt", use_container_width=True):
            st.session_state.lecture_topic = "Normal & Tangent"
            st.session_state.page = "lecture"; st.rerun()
            
    with col_l3:
        if st.button("🎓 Lecture:\nPolar Coordinates", key="lec_polar", use_container_width=True):
            st.session_state.lecture_topic = "Polar Coordinates"
            st.session_state.page = "lecture"; st.rerun()
            
    # Section B: Practice Problems (Restored Statics)
    st.markdown("---")
    st.subheader("📝 Practice Problems")
    
    # Categorize problems automatically from the JSON
    categories = {}
    for p in PROBLEMS:
    ...
    for cat_name, probs in categories.items():
        ...
        cols = st.columns(4) # 4 columns for a wider problem spread
...
# --- Page 3: Interactive Lecture Agent ---
    ...
    with col_chat:
        ...
        if "lecture_session" not in st.session_state or st.session_state.lecture_session is None:
            ...
            sys_msg = (
                f"You are a Mechanical Engineering Professor at TAMUCC. Topic: {topic}. "
                "STRICT INSTRUCTION: Respond only in English. "
                "1. Break the derivation into 5-6 small steps. "
                "2. Ask a question after each step and wait for the student. "
                "3. Use LaTeX for formulas ($$formula$$). "
                "4. For Normal Acceleration, help them derive dot product proof: T·T=1 -> T·dT/dt=0."
            )
            model = get_gemini_model(sys_msg)
            st.session_state.lecture_session = model.start_chat(history=[])
            st.session_state.lecture_session.send_message(f"Hello {st.session_state.user_name}! Let's derive {topic} formulas step-by-step. Look at the sliders. If you change the speed, how does that affect the path visually?")
